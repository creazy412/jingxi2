...
def cycle():
    ...
    while True:
        ...
        # 明天开始+30秒时间戳
        tomorrowStartTime = int(time.mktime(time.strptime(str(tomorrow), '%Y-%m-%d')) + 30)
        # 开始时间点
        ...
        if currentTimestamp > startLoopPointTimestamp and currentTimestamp < tomorrowStartTime:
            exchange()
            time.sleep(1)
